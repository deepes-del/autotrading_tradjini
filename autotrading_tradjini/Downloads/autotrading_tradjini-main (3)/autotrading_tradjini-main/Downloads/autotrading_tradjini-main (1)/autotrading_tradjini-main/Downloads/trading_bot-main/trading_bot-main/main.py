...
def start_bot(user_id: str, user_config: dict) -> bool:
    ...
    # Ensure Central Market Data Engine is running
    data_fetcher.start_market_data_engine()
    with bot_lock:
...
def _run_bot_logic(user_config: dict) -> None:
    ...
    if user_strategy == "strategy_one":
        strat_display_name = "Strategy One"
    elif user_strategy == "strategy_two":
        strat_display_name = "Strategy Two"
    elif user_strategy == "strategy_three":
        strat_display_name = "Strategy Three"
    else:
        strat_display_name = user_strategy
    ...
    # Ensure engine is started (double-failsafe)
    data_fetcher.start_market_data_engine()

    # ── Wait for Central Market Data Engine to initialize cache ────────────────
    add_log(user_id, "Connecting to Central Market Data cache...")
    while True:
    ...
    while True:
        ...
        if ist_now.time() >= EOD_SQUAREOFF_TIME and not eod_squared_off:
            ...
            if active_trade is not None:
                ...
                with bot_lock:
                    if user_id in running_bots:
                        running_bots[user_id]["active_trade"] = None
                add_log(user_id, "🛑 Trade Exited")
                clear_setup(user_id)
            else:
        ...
        if user_strategy in ["strategy_one", "strategy_two"] and global_df is not None and not global_df.empty:
            latest = global_df.iloc[-1]
            s_time = setup['time'] if setup else (latest['timestamp_ist'] if 'timestamp_ist' in latest else global_df.index[-1])

            # Logging Candle Info (Only on new candle / setup time change)
            if s_time != last_candle_time:
                last_candle_time = s_time
                t_str = s_time.strftime('%H:%M') if hasattr(s_time, 'strftime') else str(s_time)
                ...
                if setup:
                    set_setup(user_id, setup) # sync local setup dictionary for user logs/monitoring
                    if user_strategy == "strategy_two":
                        candle_size = setup['high'] - setup['low']
                        add_log(user_id, f"📊 Candle Size: {candle_size:.2f} points")
                        add_log(user_id, f"✅ Small candle valid\n🧠 Setup detected\nWaiting for breakdown below {setup['low']:.2f}")
                    else:
        ...
        # Retrieve local setup for user logging purposes
        ...
        if active_trade is not None:
            # Check if user manually squared off the position at the broker
            entry_ord_id = active_trade.get("buy_order_id", "")
            bot_exit_ids = [active_trade.get("sl_order_id", "UNKNOWN"), active_trade.get("buy_order_id", "UNKNOWN")]
            manual_exit = order_manager.find_manual_exit_order(
                user_id=user_id,
                symbol=active_trade["opt_sym"],
                entry_order_id=entry_ord_id,
                bot_exit_order_ids=bot_exit_ids
            )
            if manual_exit:
                exit_price = float(manual_exit.get("avgPrice") or manual_exit.get("price") or active_trade["entry_price"])
                exit_order_id = manual_exit.get("orderId") or manual_exit.get("orderid")
                add_log(user_id, f"👤 Manual trade exit detected at broker! Closed at ₹{exit_price:.2f} | Order ID: {exit_order_id}")
                
                close_broker_trade(user_id, exit_price, reason="MANUAL_EXIT")
                active_trade = None
                with bot_lock:
                    if user_id in running_bots:
                        running_bots[user_id]["active_trade"] = None
            ...
            if user_strategy == "strategy_three":
                # Strategy Three Index-based Exit monitoring
                opt_tok = active_trade["opt_tok"]
                opt_sym = active_trade["opt_sym"]
                trade_qty = active_trade["trade_qty"]
                opt_type = active_trade.get("opt_type", "CE")
                
                live_index_ltp = data_fetcher.MARKET_LTP_CACHE.get(user_index)
                
                # Fetch central state stoploss
                state = data_fetcher.STRATEGY_THREE_STATE[user_index][opt_type]
                current_sl = state["current_stoploss"]
                
                if live_index_ltp is not None and current_sl is not None:
                    trigger_exit = False
                    exit_reason = ""
                    if opt_type == "CE":
                        if live_index_ltp <= current_sl:
                            trigger_exit = True
                            exit_reason = f"Index crossed CALL SL ({live_index_ltp:.2f} <= {current_sl:.2f})"
                    else:
                        if live_index_ltp >= current_sl:
                            trigger_exit = True
                            exit_reason = f"Index crossed PUT SL ({live_index_ltp:.2f} >= {current_sl:.2f})"
                            
                    if trigger_exit:
                        add_log(user_id, f"🛑 Stop Loss Hit! {exit_reason}. Placing market sell order...")
                        sell_res = order_manager.place_sell_order(user_id, opt_tok, opt_sym, trade_qty)
                        
                        exit_price = 0.0
                        if sell_res.get("success"):
                            sell_order_id = sell_res["order_id"]
                            add_log(user_id, f"✅ Market sell executed | Order: {sell_order_id}")
                            
                            # Retrieve filled price
                            order_details = order_manager.get_order_status(user_id, sell_order_id)
                            if order_details:
                                order_status = str(order_details.get("status", "")).lower()
                                if order_status in ["complete", "completed", "executed"]:
                                    exit_price = float(order_details.get("avgPrice") or order_details.get("price") or 0.0)
                                    add_log(user_id, f"✅ Market sell filled price: ₹{exit_price:.2f}")
                            close_broker_trade(user_id, exit_price, reason="SL_HIT")
                        else:
                            add_log(user_id, f"⚠️ Market sell failed: {sell_res.get('message')} — MANUAL EXIT NEEDED")
        ...
        # ── Entry phase (triggered by central signals) ─────────
        if active_trade is None:
            # Check setup for periodic logging
            if user_strategy in ["strategy_one", "strategy_two"] and setup:
            ...
            # Poll for broadcast signal
            signal = data_fetcher.SHARED_SIGNALS.get(user_index, {}).get(user_strategy)
            if signal is not None:
                # Check if signal is fresh and has not been processed yet by this user
                if signal["timestamp"] > last_processed_signal_time:
                    # Enforce Trade Entry Time Window constraint: 10:00 AM IST to 3:00 PM IST
                    ist_now = datetime.datetime.now(config.TIMEZONE)
                    if not (datetime.time(10, 0) <= ist_now.time() < datetime.time(15, 0)):
                        add_log(user_id, f"⚠️ Ignored signal at {ist_now.strftime('%H:%M:%S')} - outside 10:00 AM - 3:00 PM trading window.")
                        last_processed_signal_time = signal["timestamp"]
                        continue

                    # Signal is valid for up to 60 seconds
                    if time.time() - signal["timestamp"] < 60:
                        last_processed_signal_time = signal["timestamp"]

                        # Delete signal immediately from SHARED_SIGNALS so it cannot be consumed again
                        with data_fetcher.market_cache_lock:
                            if user_index in data_fetcher.SHARED_SIGNALS and user_strategy in data_fetcher.SHARED_SIGNALS[user_index]:
                                data_fetcher.SHARED_SIGNALS[user_index][user_strategy] = None
                        ...
                        if user_strategy == "strategy_three":
                            ...
                            opt_type = signal.get("signal_type", "CE")
                            ...
                            if opt_tok and option_ltp:
                                # We store dummy/placeholder values for SL/Target on option premium
                                opt_sl_price = 0.0
                                opt_target_price = 999999.0
                                ...
                                if buy_res.get("success"):
                                    ...
                                    if order_status in ["complete", "completed", "executed"]:
                                        ...
                                        # Note: Skip placing Stop-Loss order at broker since the SL belongs to the INDEX
                                        ...
                                        with bot_lock:
                                            if user_id in running_bots:
                                                running_bots[user_id]["active_trade"] = active_trade
                                        add_log(user_id, "✅ Trade Entered")
                                        ...
                                        clear_setup(user_id)
                                        data_fetcher.reset_strategy_three_state(user_index, opt_type)
                                else:
                                ...
                                add_log(user_id, f"❌ Failed to fetch option data for Strategy Three signal")
                                clear_setup(user_id)
                                data_fetcher.reset_strategy_three_state(user_index, opt_type)
                        else:
                            setup = signal["setup"]
                            ...
                            if candle_range <= 0:
                                ...
                                if opt_tok and option_ltp:
                                    ...
                                    if not opt_tok:
                                        ...
                                        from session_manager import has_session
                                        if not has_session(user_id):
                                            reason.append("ATM token found successfully but option LTP fetch failed due to broker authentication failure.")
                                        else:
                                            reason.append(f"ATM token found successfully but option LTP fetch failed for token={opt_tok} sym={opt_sym}")
