...
def find_manual_exit_order(user_id: str, symbol: str, entry_order_id: str, bot_exit_order_ids: list[str]) -> dict | None:
    """
    Search the broker's order book for any completed SELL order for the given symbol
    that was placed after the entry_order_id and is not one of the bot's own exit order IDs.
    """
    try:
        res = _get("/api/oms/orders", user_id)
        orders = _extract_rows(res)
        ...
        # Filter orders
        for order in orders:
            ord_sym = order.get("symbol") or order.get("symbolName") or order.get("symName")
            if not ord_sym or ord_sym != symbol:
                continue
            
            side = str(order.get("side") or order.get("transactionType") or "").lower()
            if side != "sell":
                continue
            
            status = str(order.get("status") or "").lower()
            if status not in ["complete", "completed", "executed"]:
                continue
            
            ord_id = str(order.get("orderId") or order.get("orderid") or "")
            if ord_id in bot_exit_order_ids:
                continue
                
            entry_id = str(entry_order_id)
            if entry_id and ord_id <= entry_id:
                continue
                
            return order
    except Exception as exc:
        logging.error(f"[MANUAL EXIT CHECK] Error: {exc}")
    return None
