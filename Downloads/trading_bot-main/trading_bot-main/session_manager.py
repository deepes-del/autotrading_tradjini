...
def create_user_session(
    ...
    try:
        ...
        logging.info(
            f"[SESSION] Session created/updated | user={user_id} | "
            f"client={client_id} | api_key={_mask(api_key)}"
        )
...
def get_user_session(user_id: str) -> dict | None:
    ...
    Returns the full row including api_key, client_id, access_token.
    """
    try:
        res = (
            supabase.table("broker_sessions")
        ...
        if res.data:
            logging.info(f"[TOKEN_REUSE] Reusing existing session for {user_id}")
            return res.data[0]
        return None
    except Exception as exc:
...
def build_broker_ctx(user_id: str) -> dict | None:
    """
    Build a broker context dict from the active session.
    Returns {api_key, access_token, client_id} or None.
    This is the ONLY place that should construct broker request state.
    """
    session = get_user_session(user_id)
    if not session:
        logging.warning(f"[TOKEN_EXPIRED] No active broker session for {user_id}")
        return None
    ...
    if not api_key or not access_token:
        logging.warning(
            f"[TOKEN_EXPIRED] Incomplete session for {user_id} | "
            f"api_key_present={bool(api_key)} | token_present={bool(access_token)}"
        )
        return None
